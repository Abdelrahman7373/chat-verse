...
export const ServerChannel = ({channel,server,role}: ServerChannelProps) => {
    ...
    return (
        <button onClick={onClick} className={cn('group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-gray-100 dark:hover:bg-zinc-700/50 transition mb-1', params?.channelId === channel.id && 'bg-gray-200 dark:bg-zinc-700')}>
            ...
            {channel.name === 'General' && (
                <Lock className="ml-auto w-5 h-5 text-zinc-500 dark:text-zinc-400" />
            )}
        </button>
    )
}